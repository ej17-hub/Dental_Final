        ...
        private void LoadPatientDetails(int patientId)
        {
            ...
            using (var cmd = new SqlCommand(sql, conn))
            {
                ...
                using (var rdr = cmd.ExecuteReader())
                {
                    if (rdr.Read())
                    {
                        string first = rdr["first_name"] != DBNull.Value ? rdr["first_name"].ToString() : string.Empty;
                        string last = rdr["last_name"] != DBNull.Value ? rdr["last_name"].ToString() : string.Empty;
                        string mi = rdr["middle_initial"] != DBNull.Value ? rdr["middle_initial"].ToString() : string.Empty;
                        string displayName = string.IsNullOrWhiteSpace(mi) ? $"{first} {last}".Trim() : $"{first} {mi}. {last}".Trim();
            ...
            try
            {
                const string lvSql = "SELECT MAX(appointment_date) FROM appointments WHERE patient_id = @id";
                using (var conn = new SqlConnection(connectionString))
                using (var cmd = new SqlCommand(lvSql, conn))
                {
                    cmd.Parameters.AddWithValue("@id", patientId);
                    conn.Open();
                    var o = cmd.ExecuteScalar();
                    if (o != DBNull.Value && o != null && DateTime.TryParse(o.ToString(), out DateTime lastVisit))
                    {
                        label11.Text += "   Last Visit: " + lastVisit.ToString("MM/dd/yyyy");
                    }
                }
            }
            catch { /* non-fatal */ }
